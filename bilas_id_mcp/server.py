...
<body>
    <div class="card">
        ...
        <div class="step-box">
            ...
            <a class="btn btn-green" style="display:inline-block; cursor:grab;" href="javascript:(function(){try{let a=localStorage.getItem('authData')||'{}';let b=JSON.parse(a);let t=b.extendedToken||b.token||localStorage.getItem('extendedToken')||localStorage.getItem('token')||'';let o=localStorage.getItem('activeOutlet')||'';if(t){fetch('http://127.0.0.1:8765/token',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jwt:t,outlet_id:o})}).then(r=>r.json()).then(d=>{alert('✅ Bilas.id Session Token Transferred Successfully!');}).catch(e=>{alert('❌ Failed to send token to 127.0.0.1:8765');});}else{alert('❌ No Bilas.id session token found in this tab. Please log in first!');}}catch(e){alert('Error: '+e.message);}})();">🔖 Drag to Bookmarks Bar: Transfer Token to MCP</a>
